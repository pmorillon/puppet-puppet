...
if ! [ -n "$PUPPET_VERSION" ]; then export PUPPET_VERSION="3.3.1"; fi
if ! [ -n "$PUPPET_DEBIAN_SUFFIX" ]; then export PUPPET_DEBIAN_SUFFIX="-1puppetlabs1"; fi

debian_install() {
  export DEBIAN_FRONTEND=noninteractive
  lsb_release_install
  echo "\033[0;32mConfigure puppetlabs repo for APT...\033[0m"
  (cd /tmp &&
...
centos_install() {
  lsb_release_install
  echo "\033[0;32mConfigure puppetlabs repo for YUM...\033[0m"
  ...
  case $MAJOR_VERSION in
    ...
    "6")
      ...
      centos_puppet_install;;
    "*")
      echo "\033[0;33mCentos $MAJOR_VERSION no supported...\033[0m" && exit ;;
  esac
}

lsb_release_install() {
  echo "\033[0;34mLooking for lsb_release...\033[0m"
...
debian_lsb_release_install() {
  echo "\033[0;32mInstalling lsb-release via APT..\033[0m"
  apt-get -q -y -o dpkg::options::=--force-confold install lsb-release > /dev/null 2>&1
}

centos_lsb_release_install() {
  echo "\033[0;32mInstalling lsb-release via YUM...\033[0m"
  yum install redhat-lsb -y > /dev/null 2>&1
}

centos_puppet_install() {
  echo "\033[0;32mInstalling puppet $PUPPET_VERSION via YUM...\033[0m"
  yum install puppet-$PUPPET_VERSION -y > /dev/null 2>&1
}

debian_puppet_install() {
  echo "\033[0;34mAPT update...\033[0m"
...
gem_puppet_install() {
  echo "\033[0;32mInstalling Puppet $PUPPET_VERSION...\033[0m"
...
echo "\033[0;34mLooking for a previous puppet installation...\033[0m"
PUPPET_INSTALLED=$(puppet --version 2> /dev/null)
if [ $? -eq 0 ]; then
  echo "\033[0;33mPuppet $PUPPET_INSTALLED already installed.\033[0m" && exit
fi

echo "\033[0;34mPuppet version to install is $PUPPET_VERSION...\033[0m"

echo "\033[0;34mLooking for the current Linux distribution...\033[0m"
DISTRIB_ID=$(head -n1 /etc/issue | grep -Eo "(Debian|CentOS|Ubuntu)")

echo "\033[0;34m  * Linux distribution : $DISTRIB_ID\033[0m"

case $DISTRIB_ID in
  "Debian" ) debian_install ;;
  "Ubuntu" ) debian_install;;
  "CentOS" ) centos_install ;;
  * ) echo "\033[0;33mLinux Distribution $DISTRIB_ID no supported...\033[0m" && exit ;;
esac
